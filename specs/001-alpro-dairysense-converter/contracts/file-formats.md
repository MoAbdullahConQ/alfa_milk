...
- **One sheet named `Sayfa1`.** First row is exactly the following headers,
  in this order (verified against the official `Milking Import Format.xlsx`):

```
Date | Session | UnitNo | CowNumber - in dairysense number- | Milking Time -in seconds- | Milk yield | Conductivity | temperature
```
...
- **`Date`**: a real Excel **Date** cell, number format `dd/mm/yyyy`
  (displays e.g. `14/03/2001`; the value is parsed from the Alpro report,
  never wall-clock `DateTime.now()`).
- **`Milking Time`**: a real Excel **Time** cell, number format `hh:mm`
  (written from the DairySense remap so the seconds value renders as
  minutes:seconds — e.g. `225` → `03:45`, `282` → `04:42`).
- `Session` / `UnitNo` / `CowNumber`: numbers when the source value parses as
  an int, else kept as the model's string (e.g. `AM` session stays text).
- `CowNumber`: integer. Other unlisted cells: plain string/double as the
  model says.

> **Verified (2026-08-13):** a file generated by the current
> `dairy_sense_writer.dart` was **imported successfully into DairySense** —
> the headers `CowNumber - in dairysense number-` / `Milking Time -in
> seconds-`, sheet name `Sayfa1`, the `dd/mm/yyyy` Date cells, and the `hh:mm`
> Milking Time cells are all accepted by the real system. The writer constants
> live in one place at the top of `dairy_sense_writer.dart`.

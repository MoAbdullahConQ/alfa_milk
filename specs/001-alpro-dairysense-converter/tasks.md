...
- [X] T005 [P] Create `test/fixtures/` directory with a `.gitkeep` placeholder and a short `README.md` listing the 3 real files: `alpro_report.html`, `current_cow_list.xlsx`, `dairy_sense_template.xlsx` (subsequently shipped; see Phase 7 T021)
...
- [X] T008 [P] [US1] Implement `lib/features/home/data/data_sources/alpro_parser.dart`: `parseAlproReport(String html) → AlproReport` per `contracts/file-formats.md` §1 using `package:html` (`querySelectorAll('table')`, header-index based column mapping with the `normalizeHeader` helper copied from plan.md). Locate the table whose header row contains `cowno`; extract `Date`/`Session` by searching document text for labels `date`/`session` (missing label → warning, not failure). Unparsable cow number → `AlproParseError`; missing yield/dur → skip with warning (FR-014). Wrap in an `AlproParser` class with `parseFile(String htmlPath)`. **Real-file adjustments (verified against `Session1 8-8.htm.html`, `Session2 7-8.htm.html`, `session 3 7-8.htm.html`):** (a) required-header detection uses a prefix match to tolerate a trailing sort-indicator digit (the `Cow No.` header renders as `cowno1`, previously "no data table containing cow numbers found"); (b) only a truly empty yield/dur cell is skipped — a non-numeric yield (e.g. `-`) and non-time duration (e.g. `-`) mean a cow not milked that session and are kept, exported as `0` by `BuildDairySenseRowsUseCase`; (c) real **Date** extracted from the report text (`26.08.08` via `_extractPatternDate`, ISO also supported) and real **Session** extracted from "Session N" in the title (`1`/`2`/`3`, not hard-coded), with label-based extraction kept as a fallback
- [X] T010 [P] [US1] Implement `lib/features/home/data/data_sources/dairy_sense_writer.dart`: `writeDairySenseXlsx(List<DairySenseRow>, String path)` per `contracts/file-formats.md` §3 — one sheet named `Sayfa1`, first row the 8 official headers in exact order, one data row per record (FR-009). **Now writes real Excel cells verified against the official template:** `Date` as a `DateCellValue` with `dd/mm/yyyy` (parsed from the report), `Milking Time` as a `TimeCellValue` with `hh:mm` (seconds → `HH:mm` DairySense display via `CustomTimeNumFormat`, e.g. `225`→`03:45`, `282`→`04:42`), `Conductivity`/`temperature` numeric `0`, `CowNumber` int, `Session`/`UnitNo` numeric when parseable. Header/sheet-name/format constants live in ONE place at the top of this file. Wrap in a `DairySenseWriter` class with `write(List<DairySenseRow>, String path)`
...
- [X] T016 [US2] Make `test/cow_list_test.dart` pass (RED → GREEN)
...
- [X] T019 [US4] Save flow in `lib/features/home/presentation/views/widgets/report_convert_view.dart` (after T018's Continue): **native** `FilePicker.saveFile(fileName: 'DairySense_Import_<YYYY-MM-DD>_<H.mm am/pm>.xlsx', type: FileType.custom, allowedExtensions: ['xlsx'])` (FR-015 — native dialog makes the name editable, no custom UI); the user picks the destination **every** conversion and the source report is **never** used as the output path (the `outputXlsxPath: htmlPath` placeholder was removed); the write runs in `Isolate.run` via `runConversion` with the chosen path; an existing/locked file → friendly `OutputWriteFailure` message and a retry loop (FR-017). **Cancel bug fixed**: cancelling the save dialog or any conversion dialog immediately `reset()`s the cubit to idle (re-enables CONVERT), creates no file, and shows no success/error dialog, so the user can convert again without restarting. All dialog/summary text is selectable/copyable. Filename time is 12-hour am/pm (e.g. `DairySense_Import_2026-08-10_11.13 am.xlsx`). Depends on T018
- [X] T020 [US4] Error handling in `lib/features/home/presentation` + `lib/main.dart`: wrap every pipeline call and show `AlertDialog` with the typed message for `AlproParseError`, `CowListError`, `OutputWriteError`, `NoCowListError` ("Import a current cow list before converting."); NEVER show a raw stack trace to the user (FR-017, Principle IV). The data layer MUST also map an empty `cowNumbers` set to `NoCowListFailure` so the no-list behavior is unit-testable (see T018's guard and the new T026 test). Depends on T019
...
- [X] T022 [P] Run `flutter analyze` and fix all reported issues
- [X] T023 [P] Run `flutter test` — ALL tests green (unit + integration, incl. T026/T027)
- [X] T024 [P] Run `flutter build windows` — succeeds (requires VS2022 C++ workload per quickstart.md)
- [X] T025 [P] Walk through the 5 manual acceptance cases (A–E) and edge checks from `quickstart.md` §4 against `build/windows/x64/runner/Release/alfa_milk.exe`; confirm Definition of Done per quickstart.md §6
...
- The 3 real fixture files are shipped in `test/fixtures/` (`alpro_report.html`, `current_cow_list.xlsx`, `dairy_sense_template.xlsx`) and the integration test runs green on them (see note below; Constitution Principle VI: never invent formats)
- **Real-file verification (manual, 2026-08-10):** the user's real Alpro HTML reports (currently at `/home/conq/Downloads/Alfa Milk/Session1  8-8.htm.html`, `Session2 7-8.htm.html`, `session 3 7-8.htm.html`) now parse and convert end-to-end after the T008 real-file adjustments — Session1=147 records, Session2=127, Session3=140; Date extracted as `26.08.08`, Session as `1/2/3`; dry cows exported as `0`. Full pipeline write→read-back verified. The `test/fixtures/` copies and the `integration_test.dart` (T021) were subsequently completed.
- **XLSX compatibility verified (2026-08-13):** a file generated by the current writer (headers `CowNumber - in dairysense number-` / `Milking Time -in seconds-`, sheet `Sayfa1`, `dd/mm/yyyy` Date cells, `hh:mm` Milking Time cells, numeric Session/UnitNo) was **imported successfully into DairySense**. The output contract in `contracts/file-formats.md` §3 reflects this verified format. Note: on read-back the `excel` package may normalize the custom `hh:mm` format to `h:mm`; the writer test accepts both (the on-disk OOXML is always `hh:mm`).
- **T024 Windows build verified (2026-08-14):** VS2022 C++ workload present; `flutter analyze` clean; `flutter test` all green; `flutter build windows` succeeded (`build\windows\x64\runner\Release\alfa_milk.exe`). The Windows SDK version is pinned to `10.0.20348.0` in `windows/CMakeLists.txt` (10.0.22621.0 has an incomplete Desktop C++ component).
- **T025 manual acceptance verified (2026-08-14):** all scenarios A–G passed against the built EXE using the real customer files (copied into `test/fixtures/`): cow-list import (41 cows, header auto-detected), restart persistence, broken-import rejection preserving the previous list, missing-cow flow (6 missing: 138, 256, 463, 470, 475, 480 — Cancel makes no file, Continue exports found only), save dialog on every conversion, cancel-save resets state, and selectable dialog text. Edge checks (no list, zero matching cows, write error handling) also passed. The integration test's header assertion was updated to the verified DairySense header names and now runs green against the real fixtures (147 report records, 35 exported, intersection of report × list).
- **Test/analyze status:** `flutter analyze` clean; all 21 tests pass (`test/converter_test.dart`, `test/alpro_parser_test.dart`, `test/conversion_cubit_test.dart`). New regression coverage: parser dry-cow + real date/session, row-builder `-`→0 mapping, and cubit `showLoading`/`reset` cancel-behavior.
- **New requirement (added during US4):** cancelling any conversion step (missing-cow dialog, save dialog) must not leave the UI stuck loading — the cubit is reset to idle, re-enabling CONVERT, creating no file and showing no dialog. Covered by `ConversionCubit.reset()` + the `_convert` guard paths.
- **New requirement (added during US4):** all user-visible dialog/summary text is selectable/copyable via `SelectableText` (conversion summary, error dialogs, missing-cow dialog, cow-list validation snackbar).

...
- Clean architecture with two layers:
  - `lib/core/` — shared, framework-independent: `core/models.dart` (entities + typed errors), `core/utils.dart` (helpers)
  - `lib/features/` — feature modules, each with `data/`, `domain/`, `presentation/`:
    - `features/alpro_converter/` — Alpro→DairySense pipeline (`data/alpro_parser.dart`, `data/dairy_sense_writer.dart`, `domain/converter.dart`, `presentation/report_convert_view.dart`)
...
- [X] T004 Create `lib/core/models.dart` with ALL entities and typed errors exactly per `data-model.md`: `AlproRecord`, `AlproReport`, `CowList`, `DairySenseRow`, `ConversionResult`, plus typed errors `AlproParseError`, `CowListError`, `OutputWriteError`, `NoCowListError` with the exact message patterns from data-model.md §6. Plain Dart classes, hand-written `==`/`hashCode` only where tests need them, no JSON annotations. Also create `lib/core/utils.dart` with the shared helpers `normalizeHeader`, `normalizeCowNumber`, `durationToSeconds` (moved from converter per plan.md)
...
- [ ] T014 [P] [US2] Implement `lib/features/cow_list/data/cow_list_loader.dart`: `loadCowListFromXlsx(String path) → CowListLoadResult` per `contracts/file-formats.md` §2 using the `excel` package. First non-empty row = headers, normalized with the same `normalizeHeader` rule (from `lib/core/utils.dart`); first header starting with `cownum` or equal to `cow` wins; otherwise fall back to first column containing a valid int and set the `usedFallback` flag; no column yields an int → `CowListError` ("no cow numbers found"). Skip blank rows, warn on non-integers, deduplicate (FR-005). Convert `excel` v4 `CellValue` subclasses to plain values with a small helper
- [ ] T015 [US2] Implement `lib/features/cow_list/data/cow_list_store.dart`: `saveCowList(CowList)` and `getCowListFile() → CowList?` using `getApplicationSupportDirectory()/cow_list.json` via `path_provider` + `dart:convert` + `dart:io`. Missing file → null; corrupt JSON → null (never crash); overwrite happens ONLY after the new list fully validates (FR-008 — old file never destroyed by a failed import)
- [ ] T016 [US2] Make `test/cow_list_test.dart` pass (RED → GREEN)
- [ ] T017 [US2] Cow-list UI in `lib/features/cow_list/presentation/cow_list_card.dart` (extend T012's card, used from main.dart shell): on startup call `getCowListFile()` and set `activeCowList` (FR-006/FR-007 — auto-reuse after restart); "Update Cow List" button → pick `.xlsx` (`FileType.custom, allowedExtensions: ['xlsx']`) → `loadCowListFromXlsx` → if fallback column was used, inform the user (FR-004) → `saveCowList` → refresh card (count, "Last updated: <ts>", source saved/imported). On import failure: show the friendly `CowListError` message and KEEP the previous list (FR-008)
...
- Copy these helpers verbatim from plan.md into `lib/core/utils.dart` (imported by `features/*/domain`): `normalizeHeader` and `normalizeCowNumber`; `durationToSeconds` reference is also in plan.md
